...
contract FilecoinStakingContract is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() public initializer {
        ...
        interestRateHistory.push(InterestRateChange({
            rate: 50000000000000000, // initial interest rate(passed in wei for precision)
    ...
    function calculateInterest(StakeInfo memory stakeInfo) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < interestRateHistory.length; i++) {
            ...
            if (rateChange.startTime > lastTime) {
                ...
                totalInterest += (amount * interestRateHistory[i - 1].rate * timePeriod) / (factor * 365 * 24 * 60 * 60 * (10**18));
        ...
        if (lastTime < block.timestamp) {
            ...
            totalInterest += (amount * interestRateHistory[interestRateHistory.length - 1].rate * timePeriod) / (factor * 365 * 24 * 60 * 60 * (10**18));
        }
